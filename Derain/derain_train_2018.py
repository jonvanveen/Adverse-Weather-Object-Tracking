...
for epoch in range(opt.niter):
  ...
  for i, data in enumerate(dataloader, 0):
    ...
    target_cpu, input_cpu = target_cpu.float(), input_cpu.float()#.cuda(), input_cpu.float().cuda()
    ...
    if ganIterations % opt.display == 0:
      print('[%d/%d][%d/%d] L_D: %f L_img: %f L_G: %f D(x): %f D(G(z)): %f / %f'
          % (epoch, opt.niter, i, len(dataloader),
             L_img.data, L_img.data, L_img.data, L_img.data, L_img.data, L_img.data))
      sys.stdout.flush()
      trainLogger.write('%d\t%f\t%f\t%f\t%f\t%f\t%f\n' % \
                        (i, L_img.data, L_img.data, L_img.data, L_img.data, L_img.data, L_img.data))
      trainLogger.flush()
